...
// ========= DEFAULTS PARAMS ========= //
    ...
    $.ktkNavMulti = function(element, options) {
// ========= DEFAULTS PARAMS ========= //
        var defaults = {
            mobileWidth: 768,
            overlay: true,
            ...
            transformCutoff: 0,
            iconMenu: '<i class="fa-solid fa-ellipsis" aria-hidden="true"></i>',
            // ================ end flexMenu ================ //
            fixedInit: false,
            fixedId: '#ktk-nav-id',
            topSpace: 0,
...
// ========= PLUGIN EVENTS ========= //
        ...
        plugin.init = function() {
            ...
            if(plugin.settings.leftFlag)
                ...
                $(element).find('.ktk-close').on('click touchend', function() {
                ...
                $(element).find(".btn-search, .ktk-close").on("click touchend keyup", function(e) {
                    e.stopPropagation();
                    e.preventDefault();
...
// ========= FULL.SCREEN.MENU ========= //
        var fullScreenMode = function() {
            plugin.showNav = function() {
                $('.ktk-nav-full-screen').addClass(plugin.settings.animation).fadeIn(300, function(){
                    ...
                    $subElem.children('li').each(function() {
                        if(index != 0) {
                            ...
                            if($subLevelUp.children('div.mobile-level').children('ul.submenu-items').length > 0) {
                                strSubLevel += '<span class="submenu-indicator item-icon"><i class="fa fa-angle-down" aria-hidden="true"></i></span></a>'
                                    + '<ul class="ktk-full-screen-sub-level">' + subElement($subLevelUp) + '</ul></li>'
            ...
            plugin.initFullScreen = function() {
                var fullScreenWindow = '<div class="ktk-nav-full-screen ' + plugin.settings.fullScreenTheme + '">'
                    + '<div class="ktk-nav-full-screen-header">';

                $strE = $(element).find('.left-section').find('.nav-brand');
                if($strE.length > 0) {
                    fullScreenWindow += '<div class="ktk-nav-full-screen-brand">' + $strE[0].outerHTML + '</div>';
                }
                fullScreenWindow += '<button class="ktk-close t-1 ktk-close-center"></button>';
                fullScreenWindow += '</div><div class="ktk-nav-full-screen-content">'
                    + '<div class="ktk-nav-full-screen-lists ktk-scroll">';
                ...
                $(element).children('section.center-section')
                    ...
                    if(trans != null) {
                        $(this).children('ul.transform-submenu').find('> li').each(function() {
                            var tagLi = '<ul class="ktk-nav-full-screen-list">'
                                + '<li class="ktk-nav-full-screen-list-header">'
            ...
            plugin.initEvents = function() {
                ...
                $elem = $('.ktk-nav-full-screen-lists');
                $elem.children("ul").find("span.item-icon").bind("click touchend", function(e){

                    e.stopPropagation();
                    e.preventDefault();

                    if($(this).parent().siblings('.ktk-full-screen-sub-level').length > 0) {
                        if($(this).parent().siblings('.ktk-full-screen-sub-level').css("display") == "none") {
                            $(this).parent().siblings('.ktk-full-screen-sub-level').css("display", 'flex');
                            $(this).addClass("submenu-indicator-minus");
                            return false;
                        }
                        else $(this).parent().siblings('.ktk-full-screen-sub-level').css("display", 'none');
            ...
            plugin.hideNav = function() {
                $('.ktk-nav-full-screen').removeClass('zoom').fadeOut('300');
                $('html,body').css('overflow', 'auto');
            }

            var buttonHideNav = function() {
                $('.ktk-nav-full-screen').children('.ktk-nav-full-screen-header').children(".ktk-close").on("click touchend", function() {
                    plugin.hideNav();
                });
            }

            var adjustMenuPadding = function() {
                var contentHeight = $(element).find(".ktk-nav-full-screen-content").innerHeight();
                var innerElementsHeight = $(element).find(".full-screen-nav-menu, .ktk-nav-full-screen-lists, .full-screen-nav-boxes, .full-screen-nav-circles, .full-screen-nav-general").innerHeight();
                if(innerElementsHeight > contentHeight){
                    $(element).find(".ktk-nav-full-screen-content").css("padding-top", contentDefaultPaddingTop);
                }
                else{
                    $(element).find(".ktk-nav-full-screen-content").css("padding-top", (contentHeight - innerElementsHeight) / 2);
                }
            }
...
// =========  WINDOW HEIGHT ========= //
    ...
    $.fn.ktkNavMulti = function(options) {
        return this.each(function() {
            if(undefined === $(this).data('ktkNavMulti')) {
                var plugin = new $.ktkNavMulti(this, options);
                $(this).data('ktkNavMulti', plugin);
            }
        });
    };
